<?php
	...
	while ($do_venta_detalle -> fetch()) {	 
		...
		$detalle[$do_venta_detalle -> detalle_id]['cant_dev'] = $do_venta_detalle -> detalle_cant_devueltos;
		$cant_devueltos += $do_venta_detalle -> detalle_cant_devueltos;
		$cant_productos += $do_venta_detalle -> detalle_prod_cant;
	}

	$hayPaDevolver = true;
	if($cant_productos <= $cant_devueltos) {
		$hayPaDevolver = false;
	}

 	// DEVOLUCIONES
 	if($cant_devueltos) {
		$devueltos = DB_DataObject::factory('devolucion');
		$productos_cambiados = $devueltos -> getCambiadosPorVenta($do_ventas -> venta_id);
		$devueltos_pagos = DB_DataObject::factory('devolucion');
		$notas_pagos = $devueltos_pagos -> getNotasYPagosPorVenta($do_ventas -> venta_id);
		//print_r($notas_pagos);
 	}
